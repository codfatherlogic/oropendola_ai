...
def create_subscription_and_invoice(plan_id: str):
	...
	try:
		...
		end_date = frappe.utils.add_days(start_date, plan.duration_days) if plan.duration_days > 0 else None
...
def payu_success():
	...
	try:
		...
		if result["success"]:
			...
			success_url = f"/payment-success?subscription={result.get('subscription_id')}&invoice={result.get('invoice_id')}"
